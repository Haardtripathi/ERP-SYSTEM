...
const WorkbookPage = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const response = await getAllWorkbook(currentPage, itemsPerPage); // 🔥 Make sure API gets `page` & `limit`

            console.log("📌 API Response:", response.data); // Debug response

            if (response && response.data) {
                const newData = response.data.data;
                console.log("✅ New Data for Page", currentPage, ":", newData);

                setWorkbookData(newData);
                setFilteredData(newData);
                setPaginatedData(newData.slice(0, itemsPerPage)); // ✅ Ensure correct pagination
    ...
    useEffect(() => {
        fetchData();
    }, [currentPage]); // ✅ Fetch only when `currentPage` changes
    ...
    const handlePageChange = (page) => {
        if (page >= 1 && page <= totalPages && page !== currentPage) {
            setCurrentPage(page); // ✅ First, update the page number
        }
    };
    ...
    // useEffect(() => {
    //     fetchData();
    // }, [currentPage]); // ✅ Fetch new data whenever `currentPage` changes
    ...
    useEffect(() => {
        const applyFiltersAndPaginate = () => {
            let filtered = workbookData.filter((item) => {
                if (searchColumn === "all") {
            ...
            if (filterStatus !== "All") {
                filtered = filtered.filter((item) => {
                    return filterStatus === "isSent" ? item.is_sent_to_pending : !item.is_sent_to_pending;
                });
            }

            setFilteredData(filtered);

            // ✅ Only update `totalPages` when filtering
            if (searchTerm || filterStatus !== "All") {
                const calculatedPages = Math.ceil(filtered.length / itemsPerPage);
                console.log("📌 New `totalPages` from filter:", calculatedPages);
                setTotalPages(calculatedPages > 0 ? calculatedPages : 1);
            } else {
                setTotalPages(Math.ceil(totalRecords / itemsPerPage)); // ✅ Keep API `totalPages`
            }
    ...
    }, [workbookData, filterStatus, searchTerm, searchColumn, currentPage, itemsPerPage, totalRecords]);
