...
const TableRow = ({ children }) => <tr className="bg-orange-100 hover:bg-orange-200">{children}</tr>
...
const ReturnPage = () => {
    ...
    const { permissions, loading } = useAccessControl("/return")
    ...
    useEffect(() => {
        ...
        if (!permissions.page || permissions.page !== "/return") {
    ...
    const fetchData = async (page = currentPage, limit = itemsPerPage, search = searchTerm, column = searchColumn) => {
        try {
            ...
            const response = await getAllReturn(queryString)

            if (response.data && response.data.returnData) {
                const validData = Array.isArray(response.data.returnData)
    ...
    if (isLoading && returnData.length === 0) {
        return (
            <div className="flex justify-center items-center h-screen">
                <Loader2 className="h-8 w-8 animate-spin text-blue-500" />
            </div>
    ...
    return (
        <div className="container mx-auto p-8 bg-gray-50 h-full max-w-full">
            <Card className="mb-6">
                <CardHeader className="flex flex-row items-center justify-between pb-4">
                    <CardTitle className="text-3xl font-bold">Return Page</CardTitle>
                    <div className="flex items-center space-x-4">
                        {/* Refresh Button */}
                        <TooltipProvider>
                            <Tooltip>
                    ...
                    <Table>
                        <TableHeader>
                            <tr className="bg-gray-200">
                                ...
                                {/* Group 6: (Address, Shipment Type) */}
                                {(hasColumnPermission("address") || hasColumnPermission("shipment_type")) && (
                                    <TableHead>
                                        <div>
                                            {hasColumnPermission("address") && <span>Address</span>}
                                            {hasColumnPermission("address") && hasColumnPermission("shipment_type") && <br />}
                                            {hasColumnPermission("shipment_type") && <span>Shipment Type</span>}
                                        </div>
                                    </TableHead>
                                )}
                                        ...
                                        {(hasColumnPermission("date") || hasColumnPermission("date_dispatched")) && (
                                            <TableCell>
                                                <div>
                                                    ...
                                                    {hasColumnPermission("date") && hasColumnPermission("date_dispatched") && <br />}
                                                    {hasColumnPermission("date_dispatched") && <span>{item.dispatchedId?.date}</span>}
                                                </div>
                                            </TableCell>
                                        )}
                                            ...
                                            <TableCell>
                                                <div>
                                                    ...
                                                    {(hasColumnPermission("cm_first_name") || hasColumnPermission("cm_last_name")) && (
                                                        <span>
                                                            {hasColumnPermission("cm_first_name") ? item.dispatchedId?.confirmedId?.cm_first_name : ""}
                                                            {" "}
                                                            {hasColumnPermission("cm_last_name") ? item.dispatchedId?.confirmedId?.cm_last_name : ""}
                                                        </span>
                                                    )}
                                                </div>
                                            </TableCell>
                                        ...
                                        {(hasColumnPermission("address") || hasColumnPermission("shipment_type")) && (
                                            <TableCell>
                                                <div>
                                                    ...
                                                    {hasColumnPermission("address") && hasColumnPermission("shipment_type") && <br />}
                                                    {hasColumnPermission("shipment_type") && <span>{item.dispatchedId?.confirmedId?.shipment_type?.value}</span>}
                                                </div>
                                            </TableCell>
                                        )}
                ...
                {totalPages > 0 && (
                    <>
                        <div className="text-sm text-gray-600">
                            Showing {returnData.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} to{" "}
