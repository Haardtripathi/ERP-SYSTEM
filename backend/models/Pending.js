...
pendingSchema.pre("validate", async function (next) {
    ...
    if (!this.ref) {
        ...
        // Count total documents in both "Pending" and "Confirmed" collections for the current year & month
        const pendingCount = await Pending.countDocuments({
            ref: new RegExp(`^${year}${month}`),
        });

        const confirmedCount = await Confirmed.countDocuments({
            ref: new RegExp(`^${year}${month}`),
        });
